...
func renderHtmlPage(headBlock Block, input chan Block) []byte {
    ...
    result = append(result, []byte("<div style=\"width:45rem;max-width:45rem;margin:0 auto;text-align:end; background-color:#a7a8aa; padding:1.5rem\">")... )
    ...
    for blk := range input {
        ...
        switch blk.Kind {
            ...
            case BlkCode:
                result = append(result, []byte("<div style=\"position:relative;margin-top:0.5rem;margin-bottom:0.5rem\"><pre class=\"code\"><table style=\"width: 100%;border-collapse: collapse\">")... )
                if len(blk.Title)>0 {
                  result = append(result, []byte("<div style=\"position:absolute;right:1rem;top:-0.5rem;background:white;border:2px solid black;padding:0.125rem\">")...)
